...
char card[12],correctcard1[14] = "18008907FB6D";
char correctcard2[14] = "1800892F229C";
char correctcard3[14] = "19004863E5D7";
...
int main(void)
{
	...
	while(1)
	{
	   ...
	   int a=strncmp(card,correctcard1,12);
	   int b=strncmp(card,correctcard2,12);
	   int c=strncmp(card,correctcard3,12);
	    if (a<=0)
	    {
			disp_cmd(0X01);
			disp_cmd(0xC0);
			disp_string("DIVYA PRESENT");
		}		
		else if (b<=0)
		 {
			 disp_cmd(0X01);
			 disp_cmd(0xC0);
			 disp_string("AYUSH PRESENT");
		 }
		else if (c<=0)
		  {
			  disp_cmd(0X01);
			  disp_cmd(0xC0);
			  disp_string("ARTI PRESENT");
		  }
			 /* _delay_ms(1000);
			  PORTC = 0b00000010;
			   _delay_ms(1000);
